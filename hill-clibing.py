...
def get_temp(individual):
    ...
    temp_list = [measure_temperature() for _ in tqdm(range(Iteration))]
    temp_list = heapq.nlargest(20, temp_list)
    ...
    for generation in range(MAX_GENERATIONS):
        ...
        for _ in range(3):
            if random.random() < 0.4:
               new_individual = mutate(now_individual, instructions)
        shutil.copyfile(NEW_INSTRUCTION_FILE, f"{TMP_FOLDER}/{generation + 1}.s")
        new_temp = get_temp(new_individual)
        print(f"Generation {generation + 1}")
        if new_temp > now_temp:
        ...
        if new_temp > best_temp:
            ...
            print("New record!")
        print("New Tempreture", round(new_temp, 3))
        print("Current Tempreture", round(now_temp, 3))
